function results_table = PS2_comparePSD(psd1, psd2, f_array)
% function to compare 2 psd structure
%
% Inputs :
%           - psd_struct : psd.psd structure composed of N struct (N channels)
%               * channel
%               * fft_iterations
%               * psd
%           psd1 = interest file, psd2 = background (baseline or sham)
% 
%           - f_array: vector of real frequencies of the structure
%
%
% SMV : 06/03/2023 + SFS 19/06/2023
...
%% check inputs :
channels1 = {psd1(:).channel};
channels2 = {psd2(:).channel};

%% inser f_array inside the structure
for i = 1 : length(psd1)
    psd1(i).f_array = f_array;
end

for i = 1 : length(psd2)
    psd2(i).f_array = f_array;
end


chan_to_do = intersect(channels1,channels2);

f_band = [1 4; 4 8; 8 15; 15 30; 30 45];
% f_band = [4 8; 8 15; 15 30; 30 45];
% f_band = [1 4; 4 8; 8 15; 15 30; 30 45; 45 80; 80 250];

[M,f1_idx1 ]= min(abs(psd1(1).f_array-f_band(1,1)));
[M,f45_idx2 ]= min(abs(psd1(1).f_array-f_band(5,2)));
...
for i = 1 : nb_chan
    idx1 = find(strcmpi({psd1.channel},chan_to_do{i}))
    idx2 = find(strcmpi({psd2.channel},chan_to_do{i}))
...
%         plot(psd_struct1(idx1).f_array,10*log10(mean_psd))
    ...
    % FOOOF
    % FOOOF settings
    freqs = psd1(idx1).f_array;
    f_range = [f_band(1,1) f_band(4,2)];
    settings = struct();  % Use defaults
    freq_oi = freqs(f4_idx1:f45_idx2);
    idx_freq_oi = f4_idx1:f45_idx2;

    mean_spectrum1(:,i) =  mean(psd1(idx1).fft_iterations,2);
    mean_spectrum2(:,i) =  mean(psd2(idx2).fft_iterations,2);
    ...
    oscill_part_tot1 = zeros(size(psd1(idx1).f_array));
    oscill_part_tot2 = zeros(size(psd1(idx1).f_array));
    % fooof ap
    ...
    total_pow1 = sum(psd1(idx1).fft_iterations(f4_idx1:f45_idx2,:),1);           % regarder entre 4 et 45
    total_pow2 = sum(psd2(idx2).fft_iterations(f4_idx1:f45_idx2,:),1);
    percentage_band_power1 = [];
    percentage_band_power2 = [];
    %% faire fooof sur chaque fenetre
    aperiodic_param1 = [];
    aperiodic_param2 = [];
    nb_win1 = size(psd1(idx1).fft_iterations,2);
    nb_win2 = size(psd2(idx2).fft_iterations,2);
    oscill_part1 = zeros(size(psd1(idx1).f_array,2),nb_win1);
    oscill_part2 = zeros(size(psd1(idx1).f_array,2),nb_win2);
    parfor w = 1:nb_win1
        % Transpose, to make inputs row vectors
        psd = psd1(idx1).fft_iterations(:,w)';
    ...
    parfor w = 1:nb_win2
        % Transpose, to make inputs row vectors
        psd = psd2(idx2).fft_iterations(:,w)';
    ...
    for f_band_idx = 1 : size(f_band,1)
        a = a+1;
        [M,f_idx1 ]= min(abs(psd1(idx1).f_array-f_band(f_band_idx,1)));
        [M,f_idx2 ]= min(abs(psd1(idx1).f_array-f_band(f_band_idx,2)));
        ...
        percentage_band_power1(f_band_idx,:) = sum(psd1(idx1).fft_iterations(f_idx1:f_idx2,:),1)./total_pow1;
        percentage_band_power2(f_band_idx,:) = sum(psd2(idx2).fft_iterations(f_idx1:f_idx2,:),1)./total_pow2;
        ...
        log_spectrum_by_win1 = 10*log10(psd1(idx1).fft_iterations);
        log_spectrum_by_win2 = 10*log10(psd2(idx2).fft_iterations);
...
%     [h,p] = ttest2(aperiodic_param1,aperiodic_param2)
    ...
    for w = 1:size(log_spectrum_by_win1,2)
        [b,stats] = robustfit(psd1(idx1).f_array(f4_idx1:f45_idx2)',log_spectrum_by_win1(f4_idx1:f45_idx2,w));
        b1(w,:) = b;
        stats1(w) = {stats};
        all_slope1 = b1(:,2);
    end
    for w = 1:size(log_spectrum_by_win2,2)
        [b,stats] = robustfit(psd2(idx2).f_array(f4_idx1:f45_idx2)',log_spectrum_by_win2(f4_idx1:f45_idx2,w));
        b2(w,:) = b;
        stats2(w) = {stats};
        all_slope2 = b2(:,2);
    end
    ...
    if i ==1
        ...
        plot(b1(50,1)+b1(50,2)*psd1(idx1).f_array(f4_idx1:f45_idx2))
        plot(log_spectrum_by_win2(f4_idx1:f45_idx2,50));
        plot(b2(50,1)+b2(50,2)*psd1(idx1).f_array(f4_idx1:f45_idx2))

        figure; 
        hold on;
        plot(mean(log_spectrum_by_win1(f4_idx1:f45_idx2,:),2));
        plot(mean(b1(:,1),1)+mean(b1(:,2),1)*psd1(idx1).f_array(f4_idx1:f45_idx2))
        plot(mean(log_spectrum_by_win2(f4_idx1:f45_idx2,:),2));
        plot(mean(b2(:,1),1)+mean(b2(:,2),1)*psd1(idx1).f_array(f4_idx1:f45_idx2))
