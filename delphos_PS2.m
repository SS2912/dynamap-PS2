function [results_all, delphos_melttable] = delphos_PS2(input_folder, mtgs_folder, sections, varargin)

%% Analyze DELPHOS .mat results and create a table with all results for different sections and subjects
% 
% Syntax:  
%    [results_all, delphos_melttable] = delphos_PS2(input_folder, mtgs, sections)
%    
% Input format: 
...
%   - mean_rate_delphos.m, 
%
% Authors: Sara Simula (original: Feb 2023; last version: March 2023)
...
for i = 1:size(mtgs,1) % for each subject
    ...
    % 2b. for each subject, and for each section, calculate spike/osc rate via the "mean_rate_delphos.m function
        ...
        info = readinfo(sub, delphos_subtab.chan, tabledir, 'simEFdir', simEFdir);
...
delphos_melttable.Properties.VariableNames = ["subject", "chanRaw", "chanDelphos", "section", "alpha", "beta", "fastripple", "gamma", "ripple", "spike"];
